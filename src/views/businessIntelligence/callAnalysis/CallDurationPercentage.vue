<template>
  <div v-loading="loading">
    ...
    <div class="main-container">
      <div class="content">
        <flexbox justify="space-between" align="center" class="axis-title-wrap">
          <h4 class="axis-title">
            通话时长占比
            <el-tooltip class="item" effect="dark" content="通话时长占比：符合某条件通话时长的通话次数/通话总次数*100%" placement="top">
              <img src="@/assets/img/question_icon.png" style="width: 20px; height: 20px">
            </el-tooltip>
          </h4>
          <flexbox justify="flex-end" align="center" class="axis-condition">
            <p style="width: 170px">通话时长占比</p>
            <el-select v-model="callSelectVal" placeholder="请选择通话时长" @change="selectValueChange">
              <el-option
                v-for="(item, index) in callItems"
                :key="index"
                :label="item.name"
                :value="item.value">
              </el-option>
            </el-select>
            <el-input v-model="inputVal" placeholder="请输入时长" style="width: 60px; margin-right: 10px" @change="inputChange"></el-input>
            <p style="width: 150px">分钟的通话</p>
          </flexbox>
        </flexbox>
...
import {
  queryCallChart, queryCallChartDetail
...
export default {
  ...
  methods: {
     ...
     */
    searchClick(params) {
      this.postParams = { ...params, action: 'durationRatio', callDurationCondition: this.callSelectVal, callDurationValue: this.inputVal }
      this.getDataList()
      this.getRecordList()
    },
    /**
    ...
    selectValueChange(data) {
      this.postParams.callDurationCondition = this.callSelectVal
      this.getDataList()
    },
    inputChange() {
      this.postParams.callDurationValue = this.inputVal
      this.getDataList()
    }
  }
}
