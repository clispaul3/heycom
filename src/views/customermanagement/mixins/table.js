...
import { talkingList, queryPhoneByUser } from '@/api/customermanagement/call'
import { queryCallHistory, callHistoryDetail, chargeRecord } from '@/api/financeManagement'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      // 通过crmIndexRequest查询对应的接口
      if (crmIndexRequest === undefined) {
        this.loading = false
        return
      }
      var params = {}
      if (this.crmType === 'callSetting') {
        params = {
          status: this.status,
          mobile: this.mobile,
          pageType: 0
        }
      } else if (this.crmType === 'spend') {
        params = {
      ...
      } else if (this.crmType === 'callDetail') {
        params = {
          page: this.currentPage,
          limit: this.pageSize,
          date: this.date,
      ...
      } else if (this.crmType === 'history' || this.crmType === 'chargeRecord') {
        params = {
          page: this.currentPage,
          limit: this.pageSize,
    ...
    /** 获取列表请求 */
    getIndexRequest() {
      if (this.crmType === 'call') {
        return talkingList
      } else if (this.crmType === 'callSetting') {
        return queryPhoneByUser
      } else if (this.crmType === 'spend') {
        return talkingList
      } else if (this.crmType === 'history') {
        return queryCallHistory
      } else if (this.crmType === 'callDetail') {
        return callHistoryDetail
      } else if (this.crmType === 'chargeRecord') {
        return chargeRecord
    ...
    crmSearch(value) {
      this.currentPage = 1
      this.search = value
      this.talkingParams.keyword = value
