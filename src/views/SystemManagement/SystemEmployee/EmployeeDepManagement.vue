      ...
      <div class="el-password">
        <el-form
          ...
          <el-form-item label="验证码" prop="code" class="phone-code">
            <el-input v-model="resetUserNameForm.code" placeholder="手机验证码"></el-input>
            <!-- 验证码倒计时 -->
            <countDown :moblieValue="resetUserNameForm.username" :if-reset="ifReset" @sendCode="getCode" />
          </el-form-item>
          <el-form-item
            label="新密码"
            prop="password">
    ...
    <el-dialog
      ...
      <el-form
        ...
        <el-form-item
          ...
          <template v-else-if="item.field === 'code'">
            <el-input
              placeholder="请输入手机验证码"
              v-model="formInline[item.field]">
              <el-button type="text" slot="suffix" @click="getTelCode">获取验证码</el-button>
            </el-input>
          </template>
          <el-input
            v-else
...
import { usersList, depList, code } from '@/api/common' // 直属上级接口
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        code: [
          { required: true, message: '手机验证码', trigger: 'blur' }
        ],
        username: [
  ...
  methods: {
    //获取验证码
    getTelCode(){
      var tel = this.formInline.username;
      if(/\d+/.test(tel)){
        sendCode(tel, 'identity').then(res => {
          if(res.code === 0){
            this.$message.success('手机验证码已经发送');
          }else{
    ...
    // 新建用户
    newBtn() {
      this.companyIsAuth().then(status => {
          if(status){
            ...
            this.$confirm('请完成企业认证后再新建员工', '企业认证', {
              cancelButtonText: '取消',
              confirmButtonText: '查看认证状态'
            }).then(()=> {
              this.$router.push('/manager/systemidentification');
            }).catch(err=> {
              //取消
            });
          }
    ...
    // 获取验证码
    getCode() {
      sendCode(this.resetUserNameForm.username, 'identity').then(res => {
        if (res.code === 0) {
