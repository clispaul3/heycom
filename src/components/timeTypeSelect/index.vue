...
export default {
  ...
  computed: {
    ...
    defaultSelectType() {
      let res = { label: '今天', value: 'today' }
      switch (this.defaultSelectVal) {
        case 'today':
          res = this.typeOptions[0]
          break
        case 'yesterday':
          res = this.typeOptions[1]
          break
        case 'week':
          res = this.typeOptions[2]
          break
        case 'lastWeek':
          res = this.typeOptions[3]
          break
        case 'month':
          res = this.typeOptions[4]
          break
        case 'lastMonth':
          res = this.typeOptions[5]
          break
        case 'quarter':
          res = this.typeOptions[6]
          break
        case 'lastQuarter':
          res = this.typeOptions[7]
          break
        case 'year':
          res = this.typeOptions[8]
          break
        case 'lastYear':
          res = this.typeOptions[9]
          break
      }
      return res
    }
  ...
  mounted() {
    ...
    } else if (this.defaultSelectVal) { // 默认选中的值
      this.$emit('change', { type: 'default', value: this.defaultSelectType.value })
